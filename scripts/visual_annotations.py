    ...
    with open(label_path, 'r') as f:
        for line in f:
            ...
            try:
                ...
                x_right = min(img_width, x_center + width / 2)
                y_bottom = min(img_height, y_center + height / 2)

                box = [x_left, y_top, x_right, y_bottom]

                polygon = []
                # Remaining points are polygon coordinates
                polygon_points = parts[5:]
...
def visualize_annotations(image, annotations):
    ...
    for ann in annotations:
        ...
        text_x, text_y = 5, 15
        if polygon and class_id == 3 or class_id == 2:  # Skip lane and drivable area for polygons
    ...
    # Compute bounding box from polygon points
        if box and class_id != 3 and class_id != 2: 
